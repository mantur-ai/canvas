...
async function saveImagePrompt(input: GenerateImageInput) {
...
export async function generateProjectImage(input: GenerateImageInput) {
  const promptSave = saveImagePrompt(input).then(
  ...
  try {
    ...
    if (!promptResult.success) {
      logGenerationStep("image:prompt-save-after-error-failed", {
