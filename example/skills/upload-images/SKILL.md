...
- On Windows PowerShell, build JSON request bodies with hashtables plus `ConvertTo-Json`, and pass them to `curl.exe` through `--data-binary '@<file>'` or stdin. If a temporary payload file is needed, create its path with `Join-Path $env:TEMP 'upload_req.json'` and pass that variable to `[System.IO.File]::WriteAllText($payloadPath, $json, [System.Text.UTF8Encoding]::new($false))`. Never call `WriteAllText($env:TEMP\\upload_req.json, ...)`; PowerShell treats the unquoted path as code and fails with `MissingEndParenthesisInMethodCall`.
- Prefer native PowerShell JSON construction over hand-escaped JSON strings inside `powershell -Command`, especially for backend calls like `resolve-public-url` and `store-public-url`.
